...
def output_results(**kwargs):
  ...
  return
"""
class fl_prediction_engine(wq_prediction_engine):
  def __init__(self):
  ...
  def check_site_date_for_sampling_date(self, site_name, test_date, output_settings_ini):
    entero_value = None
    self.logger.debug("Starting check_site_date_for_sampling_date. Site: %s Date: %s" % (site_name, test_date))
  ...
  def run_wq_models(self, **kwargs):
    self.logger.debug("run_wq_models Starting")
    prediction_testrun_date = datetime.now()
    ...
    try:
      ...
      if data_collector_plugins_enable:
        ...
        for site in fl_sites:
          try:
            ...
            try:
              ...
              entero_value = None
              if datetime.now().date() != kwargs['begin_date'].date():
                entero_value = self.check_site_date_for_sampling_date(site.name, kwargs['begin_date'], output_settings_ini)
